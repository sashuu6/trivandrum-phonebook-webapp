...
function decrypt_web(value) {
    value = window.atob(value);
    return JSON.parse(CryptoJS.AES.decrypt(value, getCookie('web'), {format: CryptoJSAesJson}).toString(CryptoJS.enc.Utf8));
}
...
if($( ".encrypted" ).length > 0)
    Swal.fire(
        {
            title: 'Decrypting',
            html: 'Please wait while we decrypt the data in human readable form.<br><br><span class="encryption-conversion"></span>',
...
$(function () {
    ...
    setInterval(function () {
        count++;
        $( '.encryption-conversion' ).fadeOut(5, function () {
            $(this).text(makeid(Math.random(20,40)*100)).fadeIn(400);
        });
    }, 50);
});
...
setTimeout(function () {
    $( ".encrypted" ).each(function() {
        var $this = this;
        setTimeout( function(){
            var decry = decrypt_web($( $this ).text());
            $( $this ).html(decry);
        ...
        setTimeout(function () {
            if($('table').length > 0) {
                $(".datatable-init").dataTable().fnDestroy();
